...
export class ListaProductosComponent implements OnInit {
  ...
  public cambiarPagina(pag:number):void{
    if(pag >=1 && pag <= this.numPaginas){
      ...
      this.numBotones = [];
      if(pag +2 >= this.numPaginas){
        for(let i = this.numPaginas ; i >= 1 && i >= (this.numPaginas-4) ; i--){
          this.numBotones[i-1] = i;
        }
      }else if(pag-2 <= 1){
        for(let i = 1 ; i <= this.numPaginas && i <= 5 ; i++){
